      ...
      try {
        if (!isOneSignalInitialized) {
          await OneSignal.init({
            ...
            serviceWorkerPath: "/OneSignalSDKWorker.js", //通知を裏で受信するための Service Worker ファイルの場所
          });
          isOneSignalInitialized = true; //SPA遷移での二重init防止のためのフラグ
        ...
        //ブラウザの通知許可状態(OneSignal.Notifications.permission)はOneSignalのlogin/logoutとは独立してブラウザに保存される
        if (!OneSignal.Notifications.permission) {
          await OneSignal.Notifications.requestPermission();
          // 許可/拒否の結果は permissionChange リスナーで処理される
        }
      } catch (error) {
